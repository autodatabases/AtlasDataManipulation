#!/bin/bash -x
# Sample:
# ./create_entities_hivetable.sh -a id3 -n abc_hive_marcel_1 -d /tmp/tep -f rec -fq weekly -s core-banking
...
DB_TABLE_GUID=$(${ATLAS} \
  ...
  -H 'Accept:application/json' ${ATLAS_ENDPOINT}/entity/bulk -d name": "'"$DB_TABLE_NAME"'",
        "'
  {
  "entities": [
    {
      ...
      "attributes": {
        "description": "External HIVE table: '"$DB_TABLE_NAME"' stored in '"$FILE_DIRECTORY"' format '"$FILE_FORMAT"'",
